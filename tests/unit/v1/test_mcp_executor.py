...
@pytest.fixture
def mock_mcp_config():
    return {"test_server": StdioServerParameters(command="echo", args=[])}
...
async def test_mcp_executor_wrong_scheme(mock_mcp_config):
    ...
    action = ParsedAction(
        scheme="bash", server_or_cmd="run", method="", query_params={}
    )
...
async def test_mcp_executor_unknown_server(mock_mcp_config):
    executor = McpExecutor(mock_mcp_config)
    action = ParsedAction(
        scheme="mcp", server_or_cmd="unknown_server", method="tool", query_params={}
    )
