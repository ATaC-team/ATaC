        ...
        "steps": [
            {
                ...
                "type": "action",
                "action": "bash://run",
                "args": {"command": "echo 'hello runtime'"},
            },
            {
                "id": "run_echo_2",
                "type": "action",
...
async def test_runtime_variables():
    trajectory = {
        "version": "1.0",
        "variables": [{"name": "greeting", "type": "string", "value": "hello"}],
        "steps": [
            {"type": "set", "variables": {"target": "${inputs.target}"}},
            {
                "id": "run_echo",
                "type": "action",
                "action": "bash://run",
                "args": {"command": "echo '${variables.greeting} ${variables.target}'"},
            },
        ],
    }
