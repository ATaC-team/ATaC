...
def test_cli_global_cwd_flow(tmp_path):
    ...
    result = runner.invoke(cli, ["-C", cwd_str, "init", "remote_flow", "--description", "A remote workspace"])
    assert result.exit_code == 0
    assert "Initialized ATaC workspace" in result.output
    ...
    # 2. Add an input
    result = runner.invoke(cli, ["--cwd", cwd_str, "add-input", "remote_flow", "--name", "target_url"])
    assert result.exit_code == 0
    
    # 3. Add an action
    result = runner.invoke(cli, ["-C", cwd_str, "add-action", "remote_flow", "--id", "fetch_data", "--action", "bash://run"])
    assert result.exit_code == 0
