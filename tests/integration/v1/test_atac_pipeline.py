...
async def test_full_atac_pipeline():
    ...
    trajectory = {
        ...
        "variables": [{"name": "internal_count", "type": "integer", "value": 0}],
        "steps": [
            {
                "id": "write_temp",
                "type": "action",
                "action": "bash://run",
                "args": {"command": "echo '${inputs.test_word}' > temp_test.txt"},
            },
            {
                "id": "read_temp",
                "type": "action",
                "action": "bash://run",
                "args": {"command": "cat temp_test.txt"},
            },
            {
                "type": "if",
                "condition": "${inputs.loop_flag}",
                "then": [
                    {"type": "set", "variables": {"internal_count": 1}},
                    {
                        "type": "for",
                        "in": '["apple", "banana"]',
                        "item": "fruit",
                        "steps": [
                            {
                                ...
                                "type": "action",
                                "action": "bash://run",
                                "args": {"command": "echo 'Eating ${fruit}'"},
                            }
                        ],
                    },
                ],
                "else": [
                    {
                        "type": "action",
                        "action": "bash://run",
                        "args": {"command": "echo 'Skipped loop'"},
                    }
                ],
            },
            {
    ...
    assert (
        runtime_false.context.variables["internal_count"] == 0
    )  # Default from trajectory
