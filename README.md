...
- **Agent Memory**: 将 AI 解决过的任务模式以目录 bundle 形式持久化，入口为 `index.yaml`，并支持附带脚本与按关键词检索，用于引导后续任务。
...
```
> [!NOTE]
> `atac config` 命令生成的 `.atac/atac.json` 具有最高优先级，加载时会覆盖同名的环境变量。
...
* **Agent Memory**: Persist solved task patterns as searchable directory bundles with an `index.yaml` entry and optional helper scripts, giving agents reusable guidance for future tasks.
...
```
> [!NOTE]
> The `.atac/atac.json` file generated by `atac config` has the highest priority and will override the environment variable during context loading.
...
ATaC Memory stores solved task patterns as directory bundles. Each bundle lives at `.atac/.memory/<name>/index.yaml` and can include helper scripts or other local assets:
